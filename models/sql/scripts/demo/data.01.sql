insert into jsharmony.sys_user (sys_user_fname,sys_user_lname,sys_user_email,sys_user_pw1,sys_user_pw2,
  ...
  values ('Demo','User','demo@jsharmonycms.com','Content@Work','Content@Work',
...
insert into jsharmony.sys_user_role (sys_user_id, sys_role_name) 
  select sys_user.sys_user_id,sys_role.sys_role_name
    ...
    where sys_user_email in ('demo@jsharmonycms.com') and sys_role.sys_role_name not in ('DEV','SYSADMIN','CMSHOST','*'); 

update cms.page set page_author = (select sys_user_id from jsharmony.sys_user where sys_user_email = 'demo@jsharmonycms.com');
...
update cms.deployment_target set deployment_target_template_variables = replace(deployment_target_template_variables, 'localhost:', 'demo.jsharmonycms.com:');
